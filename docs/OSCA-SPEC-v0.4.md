...
- **取哪一格（`input.from`）**：`input: {ref: OBJ-201, from: CON-201.拉取待下发处置清单}`——connector 步的
  产物是 `{接口ref: 回执}` 一个字典；不写 `from` 时下游取**整份字典**（既有行为一字不变），写 `from` 则收窄
  到字典里的**那一格**。`from` 只指定「取哪个」，不表达任何条件——想在这里写 if 的那一刻，它是一条该进
  `judgments/` 的判断。`from` 指向的 ref 不在上游产物里 ＝ **fail-closed**（剧集失败并列出可选 ref），
  **绝不回落成「取整份」**；上游产物不是字典（如文本草稿）却写了 `from`，同样拒绝。lint OSCA043 静态咬同一判据。
- **产物形态（`produces.as`）**：`produces: {ref: OBJ-002, as: json}` 声明该步产出**结构化数据**
  （受限词表：目前只认 `json`；缺省即纯文本，既有包一字不变）。**只有 agent 步可声明**——connector /
  optimizer 的产物形状由执行器决定。声明 `as: json` 的步骤**必须**声明 `input`（可溯源纪律，见附录 A.4）。
  lint OSCA042。
...
- **agent**：LLM 依一次性上下文出草稿；产出注入剧集台账前过 Policy 脱敏。LLM 通道由部署环境变量配置
  ...
  **归属纪律**：草稿中依据命中判断的段落须在段末标注该判断 ID（如 `（J-0417）`）——蒸馏管道按引用
  ...
  - **结构化产出（`produces.as: json`）**：agent 步默认产出纯文本；声明 `as: json` 时改产**结构化数据**，
    供下游写步作被写内容——写步的 body 于是是**一行真正要写的数据**，而不是一坨上游读回执字典。参考实现的
    可求值受限形式：模型只输出**一份 JSON 信封** `{"说明": <给人看的人话草稿>, "数据": <交给下游步骤的
    结构化数据，对象或数组>}`（信封而非「人话段＋JSON 段」：两段输出必须靠分隔符/围栏扫描切分＝形状
    猜测器，猜错即写错内容）。三条纪律，缺一即不成立：
    ① **并存不是替换**：「说明」进剧集台账的 draft（人读面——专家终审、审批卡依据、控制台快照仍消费它），
    「数据」另存产物给管道吃；归属纪律照旧作用在「说明」（依据 guard 命中判断的段落段末标注判断 ID）。
    **脱敏是「一份原文、一份台账副本」，不是「脱」或「不脱」的二选一**（与附录 B.4 的既有对偶同构：
    `payload_digest` 绑原文、`payload_display` 只脱显示）：
    - **交给下游的产物是原文**——它是被写内容，显示脱敏不得改写待写值（预约行里的联系电话本就是合法
      字段），写审批门的 `payload_digest` 绑的正是这份原文，挂起快照的持久口径同此；
    - **进剧集台账/持久面的那一份是脱敏副本**——与连接器回执同权（回执一进台账就脱，见附录 B.3
      「注入前脱敏」）。台账要进控制台快照、日志与归档；结构化产出若是台账里唯一不脱的字段，就等于给
      PII 开了一条绕过脱敏进台账的旁路。
    - 人话「说明」照旧过 Policy 脱敏；实现记录的脱敏命中数应覆盖**进台账的两份**（人话草稿 ＋ 结构化副本）。

    ② **须可溯源上游产物**：声明 `as: json` 的步骤必须声明 `input`；剧集台账的逐步留痕须记下该产物
    **整形自哪个上游产物**（用 `from` 收窄时记到哪一格），**机器可查**，不是注释里写一句。
    **这条纪律的效力要说诚实话**：结构化产出须声明并记录上游来源（记账与审计用）；内容真实性由
    **审批门（人类可读 payload）** 与**回放体检**兜底，runner 不做语义保证。
    展开说：runner 机器保证得了的只有「这一格记着它整形自哪个上游产物」。模型有没有照抄那份上游数据、
    字段值改没改、数是不是编的，runner 一概读不出来——读得出就等于让 runner 判语义。真正兜住内容的是
    两道**点名的**门：
    - **审批门（人类可读 payload，附录 B.4）**：待写值原文脱敏后上审批卡，由人对着**看得懂的内容**拍板，
      模型编的数在这里撞人眼；`payload_digest` 绑的正是这份被拍板的原文，改一个字挑战即消费不掉。
    - **回放体检（附录 A.6，红灯率上接 A.3 ② 的 kill switch）**：判断注入前后的产出被 A/B 打分，编造在
      专家终稿面前留下红灯，红灯率越顶即停机——事后，但成规模。

    与 A6「模型只按名调用、按类型消费结果，永不写 SQL、永不猜数」的关系于是**与公理 A5 同构**：A5 说
    劝告与笼子分离——AGENT.md 的边界是**劝告**（塑造行为，模型被注入/被替换即失效），policy.yaml 是
    **笼子**（强制层，模型永不读、绕不过）。这里一样：「整形自上游、不凭空造数」是对模型的劝告，`input`
    声明与上游 ref 留痕是**记账**（谁整形自哪一格，事后查得到），而**笼子是审批门与回放体检**。
    **笼子是保证，模型自律只是劝告**——把劝告写成保证，就是给一句愿望盖机器保证的章。

    ③ **解析失败一律 fail-closed**：产出不是合法信封 JSON（非 JSON／半截／带 markdown 围栏／顶层非对象／
    缺格／夹带未知键／「数据」非对象或数组／超出实现声明的体积上限／嵌套过深）＝ 本步失败、剧集失败，
    **绝不「解析不了就退回当文本用」**——退回文本等于把结构化产出悄悄退化回纯文本，写路径重新拿一坨读
    回执上 wire。解析失败仍照记 tokens（外呼已发生，不许「产出非法就免费重试」绕过预算硬顶）。
    **「合法 JSON」按 RFC 8259 判，不按宿主 JSON 库的默认宽容度判**——实现须自己把下面三口堵上，
    否则这条纪律有名无实（三口均在参考实现上真跑复现过，一路走到审批卡、L2 挂起快照与 wire body）：
    - **规范外字面量 `NaN` / `Infinity` / `-Infinity`**：RFC 8259 的数字文法里没有它们，而 Python
      `json.loads` 默认收（`parse_constant`）。收下去的后果：审批卡显示 `{'金额': nan}`（审批人对着
      nan 拍板）、挂起快照落盘成 `{"artifacts": {"金额": NaN}}`（**一份非 Python 读者解不开的非法
      JSON 文件**）、执行器拼的 wire body 也是 `{"金额": NaN}`。**溢出成 ±inf 的数值（如 `1e999`）
      同拒**：它语法合法、`parse_constant` 拦不住，但重新编码时照样吐 `Infinity`，后果一字不差。
      等价判据：放行的产物必须能过 `json.dumps(..., allow_nan=False)`。
    - **同一 JSON 对象里的重复键**：默认行为是**静默取最后一个**（`{"金额":100,"金额":999999}` 只剩
      999999），「取哪一份」是猜，猜错即写错内容，且台账里谁也看不出还有过第一份。信封层与「数据」
      内层**同一把闸**——两层的后果同一个（都进被写内容、进审批卡、进 wire body）。
    - **嵌套深度上限须按下游倒推、并显式声明**，不能拿解析器自己的递归上限当上限：参考实现实测，
      `json.loads` 能收近万层，而最浅的下游消费者（渲染下游步骤输入的 YAML 序列化）**330 层就
      `RecursionError`**；该异常无人接住即冲出剧集执行、把剧集停在 `running`（既非 failed 也无停因）。
      那 330 还**不是常量**（随调用栈已用深度、部署侧递归上限、下游消费者数量浮动），故上限应取其
      **一个数量级以下**的余量值（参考实现取 32；一行待写数据的自然深度是 2–4 层）。下文的
      「执行器边界」把这类漏网兜成 `failed`，但**兜底不是许可**：边界只报得出「内部错误」，定点闸
      报得出「嵌套超 N 层」——人看得懂才改得了包。
...
**执行器边界（未预期异常不留 running 僵尸）**：`status` 是台账下游的分派键（对账 / 控制台 / 恢复调度 /
淘汰），`running` 的语义是「还在跑，别碰它」。故任何未预期异常冲出剧集执行器都必须在执行器**边界**收束
成终态 `failed` ＋ 人话停因，绝不许把剧集留在 `running`——既非 failed 也无停因的剧集没人收、没人报；若
它在恢复路径上，已批准的写还会静默永不兑现。四条纪律：
1. **捕获范围按后果定、不按异常类型定**：僵尸的危害与异常类型无关，边界捕**一般异常**（参考实现取
   `Exception`，含 `RecursionError` / `MemoryError`）——只补上一次撞见的那一种，等于承诺下一种意外照样
   留僵尸。
2. **不捕关停信号**（`KeyboardInterrupt` / `SystemExit` / `GeneratorExit` 一类）：它们是关停信号、不是
   剧集失败，吞掉会把 Ctrl-C 变成「剧集失败」并挡住三级停的关停语义——关停必须能穿透执行器。
3. **宽捕获 ≠ 吞**：完整堆栈进部署侧日志、台账只记异常**类型名**（`AttributeError` / `KeyError` 一眼
   看得出是编程错误、不是业务失败）、终态是 `failed` 而非 `completed` / `stopped`（失败仍是失败，不混进
   对账、不落 outcome case）。
4. **异常内文不进台账**：报错消息可能带连接串 / 密钥 / 令牌（binding endpoint、驱动异常回显的 DSN），而
   台账要进控制台快照、日志与归档——停因只留类型名，内文与堆栈留在部署侧日志。

边界是**兜底、不是许可**：能在定点 fail-closed 的（如上文结构化产出的解析闸）仍须在定点拒绝——边界只
报得出「内部错误：RecursionError」这句笼统话，定点闸报的是「嵌套超 N 层」，人看得懂才改得了包。
...
### A.7 人工发射（`fire`）响应契约（剧集绑定）

> 本条是**参考实现控制通道**的契约（不属包格式），写在此处是因为它规定了「谁能拿到自己这一发的剧集
> 身份」——界面/桥接按它绑定剧集，猜错即绑到别人的剧集上。

`fire` 命令的响应在既有 `{ok, detail}` 之上**增补**两个可选字段 `episode_id` 与 `operation_id`：

| 字段 | 类型 | 语义 |
|---|---|---|
| `ok` | bool | 发射是否被受理 |
| `episode_id` | str | **可选**。当且仅当这一发真的装配出剧集时出现，值即台账中的那一条（`episodes` 列得出、`episode <id>` 导得出）。**进程内**短展示编号 |
| `operation_id` | str | **可选**，与 `episode_id` 并列出现。同一条剧集的**跨重启唯一机器身份**（`EO-<uuid>`），`episodes` 摘要里同名字段查得到同一条 |
| `detail` | str | 人话结果 |

四条纪律：
1. **没装配就没有字段。** 闸门未唤醒、账本刷新失败、kill switch 拒唤醒、生命周期失效（Host 关停 /
   包跨代 / Aware 停用 / 未布防）一律**两个字段都不带**，也绝不以空串或占位值冒充。调用方据
   「有没有这两个字段」判断有没有剧集，**不据 `ok`**。
2. **两个字段同源同生同灭。** 它们都取自「这一发真装配出的那条剧集」，故**要么都有、要么都没有**。
   唯一允许的不对称是「有 `episode_id` 而无 `operation_id`」，且仅当实现内部的机器身份缺席时发生
   （参考实现中不可达：装配无条件铸 `EO-<uuid>`）——此时只丢缺席的那个并留错误痕迹，不许丢
   `episode_id`（剧集确实装配了，丢它就成了纪律 3 的假报），更不许以空串冒充身份。反向恒成立：
   **有 `operation_id` 必有 `episode_id`**。
3. **不许假报成功。** 未发布（关停 / 跨代失效 / 未布防 / 非 event / 派发异常）按既有语义回 `ok=false`
   ＋ 人话拒因；发射走完但未装配剧集的，`ok=true` 而 `detail` 须如实写明「未装配剧集」——「发射成功」
   不等于「有剧集」。
4. **加字段向后兼容。** 控制通道只对**请求**做顶层键白名单校验，响应原样序列化；不识别这两个字段的
   旧客户端必须照常工作。

**调用方纪律（员工触发桥等 requester 身份）**：以 `fire` 回传的 `operation_id` 作为剧集绑定与轮询的
**主键**，`episode_id` 只作展示；按时间窗/触发器反查台账降级为**兜底路径**，且兜底命中多条时必须放弃
绑定并报错，不许猜。

**编号语义提醒**：`episode_id`（`EP-xxxx`）是**进程内**唯一的短展示编号，Host 重启后从 `EP-0001` 重新
计号——**同一个 `EP-xxxx` 在重启前后指向的是两条不同的剧集**，拿它跨重启绑定即绑到别人那一发；
`operation_id` 每次装配都新铸，跨重启不复用，是唯一可作身份的那个。

*参考实现注*：Host 内部以回执对象 `osca_host.triggers.Delivery(reason, episode_id, operation_id)`
承载——`reason` 非空即未发布原因（与旧的「非空字符串 = 未发布原因」契约同义，watcher 自动发射不读
回值，行为不变），两个 id 只在真装配出剧集时有值。
...
- **其余 scheme（真实执行器）**——`sql_readonly` / `openapi` / `mcp`。参考实现内置 sqlite ro / urllib openapi
  参考适配器（按 `Executor` 协议可插拔、测 fake 后端）；生产驱动（postgres/mysql 只读角色、生产 API 网关）
  由**部署侧**按协议注入；未注册 scheme / `mcp`（W6 预留不实现）一律 fail-closed。约定契约（参考适配器与
  部署侧适配同守）：
  - **read-only enforcement**：`kind: sql_readonly` 的执行器只允许只读查询；写语义一律拒绝。写权限走
    B.4 的 `permissions.write` + 审批门，不在取数执行器里开写口。参考实现（sqlite）靠 **`mode=ro` 连接 +
  ...
  - **secret 解析**：执行器按 `binding.secret_ref` 经可插拔 `SecretResolver` 取值；**secret 值永不进包、
    ...
    值抹回执（键值同抹）。secret 走非 https 且非本地回环 → 拒（凭据不明文外发）。参考实现 env-var（部署侧可注入
    file/vault/callable 同协议 resolver）；真实生产 secret manager 的取值连通属部署侧适配。
  - **egress 授权**：真实执行器发起外呼前须过 Policy 的 egress 白名单（默认全禁）。**egress 校验主机须 == 实连
    主机**：endpoint authority 不得含 userinfo（`@`）——否则 `allowed@evil` 会校验 allowed、实连 evil（外审收口）。
  - **响应体解析：与模型产出同一把 JSON 闸**。HTTP 执行器把后端响应体解析成回执时，须过**与附录 A.4 ③
    完全相同的四把闸**（规范外字面量 `NaN`/`Infinity`、溢出成 ±inf 的数值、同一对象内的重复键、按下游
    倒推的嵌套深度上限），**且必须是同一份实现，不是同一套措辞**。理由是落点相同：读回执是**下游写步
    body 的原料**，还要进剧集台账、上审批卡（人类可读 payload）、进挂起快照、原样上 wire——从「模型产出」
    这个进口进来的非法 JSON，与从「后端响应体」这个进口进来的，最终坐在同一张审批卡、同一份快照、同一个
    wire body 里。两个进口各写一份判据的那天起，松的那个就是没堵的洞。
    - **深度上限两个进口取同一个数**（参考实现 32）：下游是同一批消费者（脱敏 → 入台账 → 渲染给下游
      步骤 → 落盘 → 上 wire），响应体侧只多不少（多出 secret 反射清洗与注入前脱敏两处），最浅的那个仍是
      330 层的 YAML 序列化。给响应体单开一个数，等于给同一个下游配两个要同步维护的旋钮。
    - **失败语义照既有取数分支走，不新开分支**：过不了闸 ＝ 回执不可信 ＝ **取数失败 ＝ 剧集失败**（与
      「非 2xx」「响应截断」同一条路）。**不得半解析、不得把原文当字符串回执、不得回空回执**——那三种
      都是把「body ＝ 一行真正要写的数据」悄悄换成一坨来路不明的东西。
    - **写路径同闸，但要认清拒的是什么**：写回执过不了闸时**写已经落地**（授权也已消费），fail-closed
      拒的是「把这份回执当结果用、记进台账当已知事实」——宁可剧集 failed 交给人接手，不让解不清的回执
      继续往下流。
    - **报错口径**：定点拒绝须说清是哪把闸、哪个字面量 / 哪个重复键（人看得懂才查得动后端）；理由取自
      响应体内容，故仍受本节 secret 条「回执与 error 同抹本次 secret」的反射清洗兜底。
- **注入前脱敏**：任何成功回执在注入剧集台账前过 Policy 脱敏（身份证号/手机号等），脱敏命中数记回执。
  **模型产出的结构化数据同权**：它进台账的那一份也是脱敏副本，交下游的产物仍是原文（附录 A.4 ③ 纪律 ①）。
...
- **人类可读脱敏 payload**：审批卡须呈现脱敏后的**人类可读**写内容（如改价参数原文），供审批人拍板；
  只给哈希摘要 = 让人对不可读的哈希拍板，违背「拍板给人」。参考实现：挑战 DTO 带 `payload_display`
  = `Policy.redact(原始 params)`（含 dict 键；PII 已抹），与绑原文的 `payload_digest` **分离**（redact 只脱显示、
  不动被写内容）；桥接渲染叠一层显示安全（键/值同包 code span 防 markdown 注入 + 超长截断）。
- **TTL 按人审时延**：授权过期窗口按 IM 人审时延设定（机制默认值仅为占位口径，部署侧按实际人审节奏重估）。
  参考实现可配：policy 顶层 `default_ttl_seconds`（包级默认）+ `approvals` 每项 `ttl_seconds`（每动作覆盖）；
  缺省/非法（非正有限数）一律 **fail-closed** 回落机制默认，绝不 fail-open 成无过期。真实人审节奏仍属部署侧适配。
...
**M8 新增（写步取参，T3）：**
- §5 衔接约定：`input.from`（从上游 connector 步的 `{接口ref: 回执}` 字典里取**那一格**；不写即取整份，
  既有包一字不变）与 `produces.as`（agent 步的结构化产出声明，受限词表只认 `json`）——lint OSCA043/OSCA042。
- 附录 A.4：agent 步结构化产出的可求值受限形式（一份 JSON 信封、人话 draft 与结构化数据**并存**）、
  可溯源纪律、解析失败 fail-closed。
- 附录 A.7：人工发射 `fire` 的响应契约（可选 `episode_id`，没装配就没有字段）。

**M8 新增（T4 批次）：**
- 附录 A.7：`fire` 响应在 `episode_id` 之外**并列回 `operation_id`**（同一条剧集的跨重启唯一机器身份
  `EO-<uuid>`），同源同生同灭、没装配就两个都没有；**调用方按 `operation_id` 绑定与轮询，`EP-xxxx` 降为
  进程内展示号**——Host 重启后 `EP-0001` 重新计号，拿它跨重启绑定即绑到别人那一发。
- 附录 A.4 ③ 纪律 ①：**结构化产出的脱敏改口径为「一份原文、一份台账副本」**（与附录 B.4 `payload_digest`
  绑原文、`payload_display` 只脱显示的既有对偶同构）——交下游的产物是原文，进台账/持久面的那一份是脱敏
  副本，与连接器回执同权；原「「数据」不脱」的写法会让结构化产出成为台账里唯一绕过脱敏的字段。
- 附录 A.4 ③ 纪律 ②：可溯源纪律的效力改成**诚实版**——声明并记录上游来源是**记账与审计**，内容真实性
  由**审批门（人类可读 payload）**与**回放体检**两道门兜底，runner 不做语义保证；与 A6 的关系**与公理 A5
  同构**（笼子是保证，模型自律只是劝告）。替换原「与 A6 无张力」的表态（当时无机器保证撑着）。
- 附录 A.4：新增**执行器边界**纪律——未预期异常必须在边界收束成 `failed` ＋ 人话停因，不留 `running`
  僵尸；捕获范围按后果定不按类型定、不捕关停信号、宽捕获 ≠ 吞（堆栈进部署侧日志、台账只记类型名）、
  异常内文不进台账；边界是兜底不是许可。
- 附录 B.3：后端响应体解析与附录 A.4 ③ 的模型产出**共用同一把 JSON 闸**（同一份实现、同一个深度上限
  32）；过不了闸 ＝ 取数失败 ＝ 剧集失败。写回执同闸，拒的是回执不是写。

**兼容性：** §0–§14 沿用 v0.3 编号与既有引用；`format_version` 升 `"0.4"`。存量包过渡期：分层三字段
缺失为 lint 警告（不硬拦），其余增量向后兼容。M8 的 `input.from` / `produces.as` 均为**可选新字段**，
不声明即与既有语义逐字相同。
