...
- **agent**：LLM 依一次性上下文出草稿；产出注入剧集台账前过 Policy 脱敏。LLM 通道由部署环境变量配置
  ...
  **归属纪律**：草稿中依据命中判断的段落须在段末标注该判断 ID（如 `（J-0417）`）——蒸馏管道按引用
  ...
  - **结构化产出（`produces.as: json`）**：agent 步默认产出纯文本；声明 `as: json` 时改产**结构化数据**，
    ...
    人话那部分照旧过 Policy 脱敏，**「数据」不脱**——它是被写内容，显示脱敏不得改写待写值（与附录 B.4
    `payload_digest` 绑原文、`payload_display` 只脱显示同一纪律）。
    ② **须可溯源上游产物**：声明 `as: json` 的步骤必须声明 `input`；剧集台账的逐步留痕须记下该产物
    **整形自哪个上游产物**（用 `from` 收窄时记到哪一格），**机器可查**，不是注释里写一句。这是公理 A6 的
    边界：模型整形的是**上游确定性取数的结果**、不是凭空造数，且写步仍过审批门——故结构化产出与 A6
    「模型只按名调用、按类型消费结果，永不写 SQL、永不猜数」**无张力**。
    ...
    **「合法 JSON」按 RFC 8259 判，不按宿主 JSON 库的默认宽容度判**——实现须自己把下面三口堵上，
    ...
    - **嵌套深度上限须按下游倒推、并显式声明**，不能拿解析器自己的递归上限当上限：参考实现实测，
      ...
      `RecursionError`**，且该异常会未捕获冲出剧集执行、把剧集停在 `running`（既非 failed 也无停因）。
      那 330 还**不是常量**（随调用栈已用深度、部署侧递归上限、下游消费者数量浮动），故上限应取其
      **一个数量级以下**的余量值（参考实现取 32；一行待写数据的自然深度是 2–4 层）。
...
`fire` 命令的响应在既有 `{ok, detail}` 之上**增补**一个可选字段 `episode_id`：

| 字段 | 类型 | 语义 |
|---|---|---|
| `ok` | bool | 发射是否被受理 |
| `episode_id` | str | **可选**。当且仅当这一发真的装配出剧集时出现，值即台账中的那一条（`episodes` 列得出、`episode <id>` 导得出） |
| `detail` | str | 人话结果 |

三条纪律：
1. **没装配就没有字段。** 闸门未唤醒、账本刷新失败、kill switch 拒唤醒、生命周期失效（Host 关停 /
   包跨代 / Aware 停用 / 未布防）一律**不带** `episode_id`，也绝不以空串或占位值冒充。调用方据
   「有没有这个字段」判断有没有剧集，**不据 `ok`**。
2. **不许假报成功。** 未发布（关停 / 跨代失效 / 未布防 / 非 event / 派发异常）按既有语义回 `ok=false`
   ＋ 人话拒因；发射走完但未装配剧集的，`ok=true` 而 `detail` 须如实写明「未装配剧集」——「发射成功」
   不等于「有剧集」。
3. **加字段向后兼容。** 控制通道只对**请求**做顶层键白名单校验，响应原样序列化；不识别该字段的旧客户端
   必须照常工作。

**调用方纪律（员工触发桥等 requester 身份）**：以 `fire` 回传的 `episode_id` 作为剧集绑定的**唯一来源**；
按时间窗/触发器反查台账降级为**兜底路径**，且兜底命中多条时必须放弃绑定并报错，不许猜。

**编号语义提醒**：`episode_id`（`EP-xxxx`）是**进程内**唯一的短展示编号，Host 重启后从 `EP-0001` 重新
计号；跨重启唯一的机器身份是剧集的 `operation_id`。

*参考实现注*：Host 内部以回执对象 `osca_host.triggers.Delivery(reason, episode_id)` 承载——`reason`
非空即未发布原因（与旧的「非空字符串 = 未发布原因」契约同义，watcher 自动发射不读回值，行为不变），
`episode_id` 只在真装配出剧集时有值。
