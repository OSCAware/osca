...
- **参照実装の状態：** 公開 Host 参照実装（M2 七 Component + M4 制御チャネル Security Kernel / Approval Challenge）、ファーストパーティ Flywheel と対話層（M3–M5）は機構完了・非公開。Software v1.0 は「機構検証可能」Release、実業務での検証は未完了
...
| M6 | 実 Connector 規約、全リンク統合、Software v1.0 Release | 機構統合完了。接続規約は SPEC v0.4 Appendix B、Software v1.0 は「機構検証可能」の境界で Release |

Software v1.0 は「機構検証可能」な Release として公開済み：確定版 SPEC v0.4、参照実装、Replay 可能な合成 Sample を含み、**事業効果の証明は含まない**。1.0 に付いていた実 Content の基準は 1.x に移行（削除ではない）：P0-A ≥ 20 件の L1 実 Judgment と一部 L3/L4 の後続利用 Evidence、Replay 可能な管理された実 Sample Ledger、本番統合である。Whitepaper 1.0 は文書版にすぎない。実 Ledger は顧客の Data 権利に従い、Raw Ledger の公開を意味しない。
