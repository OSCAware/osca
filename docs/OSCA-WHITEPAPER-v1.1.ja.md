...
- **ホワイトペーパー版：** 1.1（文書の版であり、ソフトウェアの版ではない）
- **公開日：** 2026-07-12
- **仕様ベースライン：** OSCA SPEC v0.4 確定版（2026-07-19。v0.3/v0.2 は History）
- **参照実装の状態：** 公開 Host 参照実装（M2 七 Component + M4 制御チャネル Security Kernel / Approval Challenge）、ファーストパーティ Flywheel と対話層（M3–M5）は機構完了・非公開。Software v1.1 は「機構検証可能」Release（真書き込み経路をエンドツーエンド接続、fake バックエンドで検証）、実業務・本番書き込みの検証は未完了

**言語：** [English](OSCA-WHITEPAPER-v1.1.en.md) · [简体中文](OSCA-WHITEPAPER-v1.1.zh-CN.md) · **日本語**
...
| 層 | 状態 |
|---|---|
| SPEC v0.4 | 確定版仕様を公開済み（v0.3/v0.2 は History） |
...
├── judgments/       # 初版は空でもよい
├── cases/           # 実運用後に生成
├── indexes/         # 機械生成キャッシュ。再構築可能、交付物に入らない
...
実行中の Agent はコンパイル成果物にすぎない。テキストの Package と Judgment Ledger こそがソースコードであり、モデルと Runtime は交換可能なコンパイラに当たる。顧客はそのソースコード——自分の Package と Judgment Ledger——を保持できるべきである。ただし契約、作者権限、Privacy 削除は別途 Governance を必要とする。
...
4. **Meta：** 作者、Batch、`confirmed`、`overruled`、Trust（Lifecycle State はファイル最上位の Field）。
...
#### 方法論的出自：なぜ専門家インタビューではなく修正の Capture なのか

「AI が蒸留し、人が決定する」は単なる工学的選好ではなく、認知科学に方法論的出自を持つ。
自然主義的意思決定研究の基本的発見（Klein, Calderwood & MacGregor, 1989；Crandall, Klein &
Hoffman, *Working Minds*, 2006）によれば、専門家の判断はパターン認識として存在する——
手がかりがパターンを起動し、パターンが予期と行動を導く（RPD モデル）——言語化可能な推論層を
経由しない。「どう判断していますか」と直接尋ねると、得られるのは事後的に合理化された一般論で
あり、実際の行動と一致しないことが多い。専門家が隠しているのではなく、判断そのものが語れない
のである。

Klein のチームが開発した Critical Decision Method（CDM）は三つの引き出し規律を課す：具体的な
出来事だけを語り、一般則を語らない——判断はエピソード記憶に宿り、合理化された概括には宿らない；
非定常の出来事だけを掘る——定常場面では専門家と初心者の行動は同じで、判断は例外においてのみ
姿を現す；プローブは手がかりと予期に向け、手順には向けない——「何をしたか」は手順を、「何に
気づき、何を予期したか」が判断をもたらす。

OSCA の Capture 設計はこの三規律に一対一で対応し、さらに一つの構造的優位を持つ：専門家は
ルールの陳述を求められず、機械が蒸留した Candidate に一言の裁決を下すだけである；Capture は
修正 Diff のみを取る——修正とは例外であり、まさに判断が現れる場所である；CDM が回顧的
インタビューで重要事象を再構成せざるを得ないのに対し、OSCA のデリバリーフローでは重要事象が
自然に発生する——専門家の各修正はいま起きている意思決定点であり、記憶の再構成に依存しない。

Judgment の五段構造も CDM の認知成分に逐項対応し、三箇所でインタビュー産物より堅い：

| CDM 成分 | Judgment 内の位置 | 堅さの理由 |
|---|---|---|
| 手がかり Cue | Signature の Guard | 自然言語記述ではなく機械評価可能なトリガー条件 |
| 判断 | Body（「ただし」を含む） | ノイズを抑制する負の Judgment も同権 |
| 予期 Expectancy | Replay アサーション | 実行可能な検証——Checkup が予期を一括検証する |
| 境界 Boundary | 「ただし」節と Expiry | 覆し回数に駆動される自動再審査に接続 |
| 類例 Analogue | Evidence の出生 Case | 任意フィールドではなく Lint ゲート |

予期はアサーションであり、類例はゲートである——これが判断資産とインタビューノートの分界線で
ある。誠実な注記も添える：本節が示すのは設計の出自であって効果の証明ではない。「デリバリー
フローに埋め込まれた Capture がインタビュー式引き出しより多くの真の判断を収集する」は、P0 の
実コーパスで検証されるべき命題のままである。
...
Candidate は Package 外に置く。非公開 `oscapipe` CLI は Confirm と Reject を提供するが、公開 `osca` CLI は提供しない。Rewrite は独立 Action ではなく、編集後の再 Distill である。Confirm は Lock 内で Evidence がまだ新鮮であることを確認し、J-ID を割り当て、Supersedes と Case State を更新し、Lint を実行し、一 Judgment 一 Commit で登録する。Reject 理由と時刻も残すべきだが、ファーストパーティ実装はこの構造化 Audit をまだ記録しない。
...
| Diff/Confirm UI、Console、Approval Card | | 製品適応 | M4 機構完了、非公開 | 独自 UI |
...
| M4 | Diff/Confirm、運用、Approval UI | 機構完了、ファーストパーティ非公開 |
| M5 | Creator Interview Assistant、Editor | 機構完了、ファーストパーティ非公開 |
| M6 | 実 Connector 規約、全リンク統合、真書き込み経路のエンドツーエンド接続、Software v1.1 Release | 機構統合完了。接続規約は SPEC v0.4 Appendix B、真書き込み経路（承認ループ ＋ 実 Executor ＋ 回復可能 Episode）は fake バックエンドで検証、Software v1.1 は「機構検証可能」の境界で Release |

Software v1.1 は「機構検証可能」な Release として公開済み：確定版 SPEC v0.4、参照実装、Replay 可能な合成 Sample、および真書き込み経路のエンドツーエンド接続（承認ループ ＋ 実 Executor ＋ 回復可能 Episode、fake バックエンドで検証）を含み、**事業効果の証明も本番書き込み検証も含まない**（後者は 1.x／デプロイ側）。1.0 に付いていた実 Content の基準は 1.x に移行（削除ではない）：P0-A ≥ 20 件の L1 実 Judgment と一部 L3/L4 の後続利用 Evidence、Replay 可能な管理された実 Sample Ledger、本番統合である。Whitepaper 1.1 は文書版にすぎない。実 Ledger は顧客の Data 権利に従い、Raw Ledger の公開を意味しない。

保守者はこれとは別に、現行システムが消化できない事実を原形のまま保存し、長期的な Guardrail と
管理された Shadow Comparison を検討する内部 R-line Design を探究している。これは現時点で
OSCA SPEC、公開 Profile、実装済み Capability のいずれでもない。Residual Ledger、四つの Gauge、
Shadow Comparison、Quarterly Review を、本書の提供 Commitment または既存 Evidence と解釈してはならない。
...
これは実装上の小話ではない。Open Specification は Code、Test、Failure Path から修正を受けなければならない。Lint は 24 の主要規律を機械化するが、SPEC 全体の証明ではない。自然言語 Guard、一部 Kind、History 上の「削除・再利用禁止」は実装と Git Transaction も必要である。
...
v0.4 は確定版だが、独立認証機関や正式 Conformance Suite はない。第三者は Package Version、Runtime Semantics Snapshot、対応 Trigger/Executor、Guard 処理、Replay Evaluator を宣言すべきである。将来の Conformance は、正負 Package、Lint Result、Runtime Event Trace、Policy Failure、固定 Mock Replay、Upgrade を比較し、Class 名や Log 文言を比較しない。
...
P0 に算入する Judgment は少なくとも次を満たす。

- Sample や合成ではなく、許可された実作業から生まれる。
- Evidence、Author、Time、Context を保持し、Episode 由来なら当時の Active Judgment Set も保持する。
- 権限専門家の Confirm、Lint、Independent Commit を経て登録される。
- `confirmed: 0` と Provisional Trust で登録され、Replay を持つ。
- Copy や同義分割で水増ししない。

Expiry は推奨であり、必須の Replay とは別項目である。口述 Rule が確定するまで、口述 Case は補助 Evidence のみで、単独では出生も P0 算入もできない。Outcome Attribution と実行可能 Replay Profile が確定するまで、Outcome Case も補助のみである。P0-A の Judgment は少なくとも一つの Replay 可能な Expert Diff Case を含む。
...
- **Reject Audit：** ファーストパーティは構造化された Reject 理由と時刻をまだ保存しない。
...
| Entry | 用途 |
|---|---|
| [OSCA SPEC v0.3](OSCA-SPEC-v0.3.md) | History（v0.4 全文に統合済み） |
| [OSCA SPEC v0.4](OSCA-SPEC-v0.4.md) | 確定版：Package Format、Ledger 規律、Runtime/Settle/Replay/Health Semantics |
...
| [歴史的 v0.1 拡張 Draft](OSCA-WHITEPAPER-v0.1.zh-CN.md) | 設計背景のみ。状態と結論は v1.1 を優先 |
