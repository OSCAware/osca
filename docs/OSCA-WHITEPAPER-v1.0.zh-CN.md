...
客户应能持有自己的包和 Judgment Ledger。这个设计目标不自动解决合同、作者授权和隐私
删除，真实部署仍需相应制度。
...
4. **Meta：** 作者、批次、状态、`confirmed`、`overruled` 与 Trust；
...
一个 Commit 入账。Reject 也应保留原因和时间；第一方
实现的结构化拒绝审计仍不完整。
...
| Diff/确认、控制台、审批卡 | | 产品化适配 | M4 尚未完成 | 可自研界面 |
...
| M4 | Diff/确认、运营与审批三种界面 | 未完成 |
| M5 | Creator 访谈助手与配置编辑器 | 未完成 |
| M6 | 真实 Connector 约定、全链路生产集成、1.0 产品发布 | 未完成 |

软件 1.0 只有在 P0-A 获得至少 20 条 L1 真实 Judgment 和部分 L3/L4 后续使用证据、M4～M6
完成，并交付规范、参考实现和可回放的受控真实样例账本后才成立。白皮书 1.0 只是本设计
文档的版本；真实账本受客户数据权利约束，不等于公开原始账本。
...
一条计入 P0 的真实 Judgment 至少应：来自授权真实工作而非样例/合成数据；保留 Evidence、
作者、时间和情境；若出生于 Episode，还要保留当时活动 Judgment 集；经有权专家 Confirm
Candidate；通过 Lint 和独立 Commit；入账时 `confirmed: 0`、Trust 为 Provisional；带 Replay；
没有复制或同义拆分凑数。Expiry 建议填写，不与强制 Replay 混为一项。在口述证据规则定稿
前，口述 Case 只能作为补充 Evidence，
不得单独出生或计入 P0 的真实 Judgment。在 Outcome 归因与可执行 Replay Profile 定稿前，
Outcome Case 同样只能作为补充 Evidence；计入 P0-A 的 Judgment 至少包含一个可回放的专家
Diff Case。
...
| [历史 v0.1 扩展草稿](OSCA-WHITEPAPER-v0.1.zh-CN.md) | 仅供设计背景；状态与结论以 v1.0 为准 |
