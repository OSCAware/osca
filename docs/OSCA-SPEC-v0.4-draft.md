...
- **kill_switch**（policy.yaml）：可求值形式两种。
  ...
  {"generated_by": "…", "at": "<ISO 时间>", "model": "<体检所用模型>",
   "ledger_head": "<体检针对的账本 git HEAD>",
   "total": 9, "green": 7, "red": 1, "error": 1, "red_rate": 0.125,
   "judgments": {"J-0417": {"light": "green", "assertions": 2}}}
  ```

  判定数据源是**整数计数**（`red × 100 > X × (green + red)`，交叉相乘无浮点舍入）；
  `red_rate` 是给人看的派生字段，与计数矛盾即档案不可信。error（不可回放：证据非
  diff 物种等）单列不入分母；**可判数 0（green+red==0）= 健康不可判（unavailable）**
  ——体检不发布档案（不覆盖上一份可判定档案），运行框架不得当 0% 处理。
  体检以账本 git HEAD 为版本快照：开始与发布前核对，中途落账即拒绝发布；发布为
  原子替换（临时文件 + fsync + rename）。运行框架校验完整契约（字段/计数一致性/
  与当前账本 HEAD 一致），任一不过按档案不可用处理——条件不生效留痕：数据可用性
  缺口走保守默认，与配置形状非法（停机）是两回事；档案**新鲜度不校验**，重跑体检
  由部署侧钩子/巡检保证。
