...
- **发布日期：** 2026-07-12
- **规范基线：** OSCA SPEC v0.4 定稿（2026-07-19；v0.3/v0.2 为历史版本）
- **参考实现状态：** 公开 Host 参考实现（M2 七组件 + M4 控制通道安全内核/审批挑战）、第一方反馈飞轮与交互层（M3–M5）机制完成（私有）；软件 v1.1 为机制可验发布（真写路径端到端接通，测 fake 后端），真实业务与生产写验证尚未完成
...
私有反馈飞轮与交互层已有维护者测试与合成夹具支持。真实 Connector 执行器、生产环境接通和业务验证仍未完成：
...
| 层 | 状态 |
|---|---|
| SPEC v0.4 | 已定稿的当前规范（v0.3/v0.2 为历史版本） |
...
| 专家界面、Creator（第一方私有） | 机制完成；产品化打磨与生产接通尚未完成 |
...
| M6 | 真实 Connector 约定、全链路集成、真写路径端到端接通、软件 v1.1 发布 | 机制集成完成；对接约定入 SPEC v0.4 附录 B；真写路径（审批闭环 ＋ 真实执行器 ＋ 可恢复剧集）测 fake 后端；软件 v1.1 以「机制可验」口径发布 |

软件 v1.1 已按「机制可验」口径发布：交付物是规范 v0.4 定稿、参考实现、可回放的合成样例
（`osca replay` 可亲验判断把输出从改前移向改后），以及真写路径端到端接通（审批闭环 ＋ 真实
sql_readonly/openapi 执行器 ＋ 可恢复剧集，测 fake 后端），**不含业务效果证明、也不含生产写验证**
（后者属 1.x／部署侧）。原定于 1.0 的真实
内容门槛整体移至 1.x（不删）：P0-A 至少 20 条 L1 真实 Judgment 与部分 L3/L4 后续使用证据、
可回放的受控真实样例账本、生产环境接通——在此之前一切指标均为机制口径。白皮书 1.1 只是
...
目前机器化 24 条关键纪律，但仍不是完整 SPEC 证明：自然语言 Guard、部分 Kind 字段、历史
...
v0.4 已定稿，但当前没有独立认证机构或正式 Conformance Suite。第三方应声明 Package
...
| 入口 | 用途 |
|---|---|
| [OSCA SPEC v0.4](OSCA-SPEC-v0.4.md) | 当前定稿：包格式、账本纪律与运行/Settle/Replay/健康档案语义 |
| [OSCA SPEC v0.3](OSCA-SPEC-v0.3.md) | 历史版本（v0.4 已并入其全文） |
