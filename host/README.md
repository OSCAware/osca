...
**M6-W5 起真写线：** 传写 params（D1，摘要绑真实被写内容）+ 可恢复剧集（D2a，写命中审批门 →
挂起 → approve 恢复兑现 / deny 回落）——真写闭环通到 L1（mock 口径）；真实系统写 + 持久化的边界
见「M4-W3 审批挑战 + M6-W5 可恢复剧集」节。
...
- **W5-D2a 可恢复剧集**：写命中审批门 → 剧集**挂起**（`suspended_pending_approval`，释放
  线程）→ approve 事件到达 → 从审批步**恢复重试消费**、mock 写执行器落地；deny / 过期 → 回落
  保守默认（不写）+ 上报。挑战绑 episode_id，恢复在**同一剧集**内兑现（重跑=新剧集不认）。
  「审批决定先到、挂起登记后到」的丢唤醒窗由登记侧复查自愈 + 惰性清扫双重堵死。
- **措辞纪律**：mock 写执行器落地 = **审批闭环机制通，非真实系统写验证**——真实
