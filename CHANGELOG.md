...
## [架构深化 · 包代际与剧集生命周期] - 2026-07-26
- CLI lint、pack 与 Host load 统一消费不可变包快照；lint 及 pack 现在也拒绝根 `indexes/`
  内的符号链接，不再透过任何包内链接读取内容。
- 挂起剧集在装配时钉住包代际；旧指纹口径无法证明同代的存量 L2 快照在升级后会 fail-closed
  作废，并留下明确日志。
- Episode 终态统一释放预算状态，Policy 审计内存有界；共享 watcher 的慢订阅不再阻塞其他订阅。
...
## [Review 修复 · M6 外审（GPT）· 3 P1 收口] - 2026-07-21
v1.1 后 GPT 全量 Code Review（3 P1 + 5 P2；缺口集中在不可信输入边界 / 异步隔离 / 硬预算强制点），本轮全修：
- **[P1] 路径越界（不可信输入边界）**：`impl` SQL / mock 取数固件 / mock LLM 固件三处都以包内 manifest 声明
  拼路径——`../`、绝对路径、包内符号链接可把读引出包根/固件目录（宿主机任意可读文件被当 SQL 执行 / 当取数
  结果注入剧集）。修：resolve 后强制留在包根/固件目录内，越界一律拒绝（+5 条回归：相对/绝对/符号链接逃逸）。
- **[P1] 事件循环阻塞（异步隔离）**：watch 轮询与触发投递（账本刷新=全量 lint+索引重建、precondition 经
  Connector 真取数 urllib timeout 10s）原先直接跑在 asyncio 事件循环上——一次外呼/慢盘压住控制通道
  （status/stop/审批）。修：deliver 协程化、重活全部 `asyncio.to_thread` 下线程；同包投递按包 asyncio 锁
  串行（保住账本非阻塞 flock 旧序，防并发刷新互踩误拒唤醒）；`fire` 命令仍等投递完成（响应语义确定）。
  随行：挂起快照 L2 持久（全包指纹 + fsync）与装载重挂（读盘 + 指纹）同下线程；落盘期间决定已到 →
  迟到快照按状态复查即刻删盘作废（不留陈旧快照在重挂时复活已决挑战）。
- **[P1] 预算绕过（硬预算强制点）**：tokens 记账源是网关**自报**的 `usage.total_tokens`，未校验——负数直接
  冲减已用额度（硬顶变软）、非整数炸记账、旧 `or` 逻辑只兜 0 不兜负。修三层：`osca_cli.llm` 源头清洗
  （非正整数一律回落字符估算）；runner aware 硬顶侧钳制（可插拔注入 llm 兜底）；policy `charge_tokens`
  强制点自持第二道闸（非法上报按 0 计 + 审计留痕，绝不接受冲减）。
- **[P2 随行] 重挂计数自防**：挂起快照 per_episode 计数损坏（非非负整数）→ 与结构不符同口径**丢弃快照**
  （fail-closed：按 0 回灌会洗掉 INV-7 硬顶，硬转 int 会炸整个装载）。
- 门禁全绿：host 315 passed（+5 回归）+ cli 153 passed（+2 回归）；两仓 ruff check/format 双绿；
  两样例包 lint 0 错误 0 警告。

## [Review 修复 · M6 外审复审（GPT）· 4 P1 + 7 P2 收口] - 2026-07-21
复审判定首轮修复「方向正确、强制点与异步生命周期未闭合」——剩 4 P1 + 7 P2，本轮全修：
- **[P1] 非法 token 上报仍绕硬预算**：首轮「按 0 计」只防冲减、仍许零成本无限过顶。改三处：runner 对
  非法上报**回落字符估算**（看得见 prompt/产出，与 OpenAICompatLLM 同口径）；policy `charge_tokens`
  强制点**fail-closed 直接拒**（看不见内容无法估算，绝不按 0 记账）；首轮固化错误期望的测试反转
  （零上报 + max_tokens=1 必触顶、连环 charge(-1) 必拒）。
- **[P1] 旧 generation 投递可唤醒 reload 后的新包**：投递重活下线程后，unload+同 id reload 可发生在任一
  await 期间——旧投递继续旧 Gate 裁决、再从注册表取到新包装配。修：进锁后按**对象身份**取齐
  gate/policy/loaded 三件套，每个 await 返回后 CAS 复核（reload 必造新对象），失配整体放弃；
  `_deliver_locks` 不再随 unload pop（pop 会让新旧代各持一把锁、互斥失效）。
- **[P1] 挂起快照「真写后复活旧 pending」崩溃窗**：首轮「落盘后按状态补删」关不住 crash window
  （delete 在文件未落时扑空 → 真写落地 → 迟到快照落盘 → 补删前崩溃 → 重挂重批**重复写**），且 unload
  期间的统一补删会误删应留盘重挂的快照。改存储层闭合：SuspensionStore persist/delete 按 operation_id
  **串行 + 删除世代令牌**——delete 世代 +1，迟到落盘在锁内复核令牌失配即**弃写**（作废先于写盘，
  不靠事后补删）；unload 不 delete，快照照常留盘（保留/作废按调用方语义区分，不再看 status 猜）。
- **[P1] 异步重挂跨代发布**：`_reattach_suspensions` 读盘/指纹线程期间 unload/reload/stop 可发生——
  线程返回后按对象身份 + Host 状态 CAS，失配即放弃、不向台账/_suspensions/挑战库写任何状态。
- **[P2] fire 出全局命令锁**：慢投递进行中 status/stop/approve 不再整体排队（实测
  status_blocked_behind_fire 已反转）；fire 仍等投递完成（响应语义不变）。
- **[P2] lint/运行时同判据**：新增 `package.resolve_in_root`（包内受限路径单一真理源），OSCA024 对
  绝对路径/`../`/符号链接越界报 **error**（此前 lint 全绿、pack 出运行时必拒的死包）；Host impl/mock
  固件/mock LLM 固件三处改共用判据。
- **[P2] 二进制伪装 .yaml 不再 traceback**：load_package 捕 UnicodeDecodeError/OSError 转 parse_error，
  OSCA003 稳定报告（PNG 改名 .yaml 此前直接炸穿 lint/pack/load）。
- **[P2] performer 受限语法**：新增 `triggers.parse_performer`（lint OSCA040 与 runner 共用）——锚定
  开头关键词 + 修饰形（`agent + judgments`/`human（…）`），子串匹配废止（`not-a-connector` 不再当
  connector）；lint 对非法 performer 报 error。
- **[P2] optimizer 拒 NaN/Infinity**：float 转换后逐项 `math.isfinite`，非有限数不进排序（NaN 此前可
  毒化排序被选为 selected）。
- **[P2] max_minutes 传导为单次调用硬顶**：runner 算剩余 deadline 传入 `llm.complete(timeout=)`（只剩
  数秒不再吊默认 120s 外呼）；OpenAICompatLLM/MockLLM 补 timeout 契约，可插拔 llm 未声明该参数则不传
  （有界执行归其实现，诚实标注）。
- **[P2] CI 补写样例**：lint job 加 `oper-dispatch.osca`；新增公仓端到端 `test_write_e2e.py`——写样例包
  打真实执行器 + fake 后端（真实 sqlite ro 取数 → 挂起 → approve 恢复真 POST 落地（server 侧捕获被写
  内容 + 脱敏）/ deny 回落零写）。
- **竞态测试补齐（复审点名缺口）**：可控屏障时序 5 条——慢刷新中 unload+reload 旧投递不发布 /
  persist-approve 竞态迟到快照不落盘 / persist-unload 竞态快照保留可重挂 / 重挂读盘中 unload 不发布 /
  慢 fire 中 status 即时返回。
- 门禁全绿：host 327 passed（+12 回归）+ cli 157 passed（+4 回归）；两仓 ruff check/format 双绿；
  两样例包 lint 0 错 0 警。

## [Review 修复 · M6 外审三审（GPT）· 2 P1 + 5 P2 收口] - 2026-07-21
三审判定跨代隔离与 max_minutes 仅部分收口、并指出 fire/stop 生命周期竞态，本轮全修：
- **[P1] fire/stop TOCTOU**：fire 短锁只查一次 RUNNING，慢投递期间进入 DRAINING 仍会发布新剧集
  （违反「迟到发布必见 tombstone」）。修：**HostState 纳入投递 CAS**（进锁即查 + 每个 await 后复核）；
  deliver 回「未发布原因」，**失效的人工 fire 如实报失败**（不假报成功）。回归：慢投递中
  `_begin_draining` → 零剧集、零 Connector 调用、fire 响应 not-ok。
- **[P1] 旧代 precondition/watch 调用新代 Connector**：Gate/poller 只捕 package_id、执行时动态取
  `self.proxies[pid]`——旧投递在外呼中挂起、unload+reload 后恢复会拿到**新代**代理（对新 binding
  陈旧外呼，误指写接口还会在新 Policy 长出无主审批挑战）。修：precondition 与 watch poll **捕获
  本代 proxy**（Gate 构建/Subscription.poll 注入），unload 即 revoke 本代 policy——旧代外呼在授权层
  必拒，fail-closed 无竞态窗（不是外呼后 CAS）。回归：precondition 外呼中 unload+reload → 新代
  Connector 零调用；旧 watch poll 跨代零调用。
- **[P2] 恢复删盘出事件循环**：`_schedule_resume` 同步 delete 会等一次 fsync（与在途 persist 争锁，
  存储故障无上界）——改**异步恢复状态机**（删盘 to_thread → CAS suspended→running → 起写线程），
  `_resuming` 防同剧集双恢复；崩溃安全序（delete-before-write）不变。回归：慢 delete 下 sweep
  同步返回 <0.2s。
- **[P2] store 注册表有界**：per-operation 锁/删除世代永久增长——改**在途凭据引用计数**
  （begin_persist/persist/delete 各持一票，归零整条回收；abandon_persist 兜指纹失败路径）；
  删除世代 tombstone 恰好活过在途 persist。回归：200 轮终态循环后注册表清零 + begin-delete-persist
  乱序仍弃写。
- **[P2] 符号链接环 + 判据真共用**：`Path.resolve` 链接环在 ≤3.12 抛 RuntimeError——`resolve_in_root`
  捕 (OSError, RuntimeError, ValueError)；SQL 执行器与 Connector mock 弃手写判据，**四处**（lint
  OSCA024 / SQL impl / mock 固件 / mock LLM）全走同一 helper。回归：lint/执行器/mock/MockLLM 四条
  链接环用例零 traceback。
- **[P2] performer 全语法 fullmatch**：前缀匹配放行任意垃圾后缀（`agent !!!` 全绿）——改整串
  fullmatch：裸关键词 / `+ 修饰词` / **闭合**中英文括注；垃圾后缀、未闭合括注一律拒。
- **[P2] max_minutes 强制 timeout 契约**：只认显式 `timeout` 形参漏掉 `**kwargs` 适配器、签名不可
  内省会炸穿、旧适配器仍无限阻塞——改：`**kwargs` 同样满足契约并真收 timeout；签名内省异常判不
  支持；**max_minutes 在而通道无契约 → fail-closed 拒绝发起**（绝不 fail-open 无界外呼）；
  无 max_minutes 不要求契约（不误伤旧适配器）。
- 门禁全绿：host 339 passed（+12 回归）+ cli 160 passed（+3 回归）；两仓 ruff check/format 双绿；
  两样例包 lint 0 错 0 警。

## [Review 修复 · M6 外审四审（GPT）· 1 P1 + 2 P2 收口] - 2026-07-21
四审确认三审主体有效，剩余问题集中在新增异步恢复状态机自身，本轮全修：
- **[P1] DRAINING 期间恢复删盘仍启动写执行 + shutdown 漏等子任务**：`_resume_after_delete` 只查
  episode.status，删盘线程期间进入 DRAINING 时剧集仍 suspended → CAS 成功推进 running 并派生逃出
  shutdown 快照的子任务（Host 报 STOPPED 而写执行还活着，可能在 unload/revoke 前恢复消费已批挑战
  真写）。修三处：**删盘前后都做完整生命周期 CAS**（HostState + loaded/proxy/policy 代际身份 +
  policy 未 revoke），失效即放弃、绝不起执行（删盘前失效快照保留、删盘后失效 fail-closed 丢写）；
  写执行改**内联 await**（恢复任务自身在 _episode_tasks，shutdown 等到全程，无子任务逃逸面）；
  `_shutdown` 改**循环重拍快照直到任务集为空**（60s 总限）。回归：slow delete → `_begin_draining` →
  release → STOPPED 时零存活任务、episode 未推进 running、零写。
- **[P2] 重挂四处 delete 仍同步阻塞事件循环**：`_reattach_suspensions` 清理过期/损坏/版本失配/计数
  损坏快照时同步 store.delete（与旧代在途 persist 争 fsync）。改**三段式**：事件循环纯裁决（零文件
  操作）→ 丢弃项**线程批量删** → 删后再代际 CAS → 发布重挂。回归：慢 delete 期间事件循环心跳 <0.2s
  （旧实现死锁/被压 350ms+）。
- **[P2] abandon_persist 双释放 ABA**：裸整数令牌 + 按 operation_id 释放 + `max(0,·)` 钳制——persist
  内部 OSError 自归还后 Host 再 abandon，第二次释放偷走并发 delete 的票、提前回收条目，同 id 新状态
  **删除世代归零**，迟到 persist 复活已作废快照（重新打开存储层快照复活面）。改 **PersistTicket
  一次性凭据**：绑定具体状态对象 + 幂等释放（registry 锁内 exactly-once），释放校验状态身份、绝不动
  别代条目；persist 错票直接拒。回归：双 abandon 不偷票 + persist OSError→abandon→delete 持票→新代
  begin 全链路（世代不归零、迟到 persist 弃写、注册表回落清零）。
- 门禁全绿：host 343 passed（+4 回归）+ cli 160 passed；两仓 ruff check/format 双绿；两样例包
  lint 0 错 0 警。

## [Review 修复 · M6 外审五审（GPT）· 1 P2 收口] - 2026-07-21
五审确认四审全部有效，剩 PersistTicket 一条：释放幂等但**使用**未锁一次性——abandon 后的旧票仍持
旧 `_OpState` 与旧世代（0），delete 用新状态递增世代并回收后，拿旧票 persist 校验的是旧状态里的
`delete_gen == token == 0`，能在注册表全空时**复活已删快照**；同票双用同理。本轮收口：
- **PersistTicket 状态机**：PENDING（begin 发出，可 abandon）→ CLAIMED（persist 在 registry 锁内
  **原子校验并认领**）→ RELEASED（终态）。认领校验四件套：票属本 store / 状态为 PENDING /
  `_ops[opid] is ticket.state`（未脱离注册表）/ operation_id 相符——任一不过拒绝且**不建文件**。
- **abandon 只作用于 PENDING**：CLAIMED（persist 使用中/其 finally 自归还）与 RELEASED 一律 no-op
  ——用过的票不被洗回可用；释放统一走 `_release_ticket(allowed=…)` 错态 no-op。
- 回归 +4（按五审点名）：begin→abandon→delete→旧票 persist 拒绝且零快照；同票串行双用第二次拒绝
  （首写内容不被覆写）；跨 store 票同 opid 拒绝零残留；persist 认领后异常自归还 → abandon no-op →
  终态票不可再用。
- 门禁全绿：host 347 passed（+4 回归）+ cli 160 passed；两仓 ruff check/format 双绿；两样例包
  lint 0 错 0 警。

## [Host M7-W4 · 部署侧 egress 增补口 egress_extra] - 2026-07-23
真实 egress 允许 host 是**环境特定**的（同 binding 端点不入公共包）——给 Host 加部署侧注入口：
- **deployment spec 加可选 `egress_extra`（字符串列表）**：`host._load` 传入 `PolicyInterceptor`；
  校验后并入 policy 的 `egress_allow`——**不覆盖包内 `egress.allow_domains`，只增补**。
- **fail-closed**：非 list / 含非字符串 / 空串一律弃用不并，绝不静默放宽 egress（与 allow_domains 同纪律）。
- 动机：此前 `egress_allow` 只从包内 policy 建、无部署侧注入口——真机对真实写后端 host（回环仿真 / 生产
  网关）一律拒，是真写落地被卡死处；亦闭合 Creator 生成写包 egress 占位/空的部署侧注入需求。
- 门禁：host 413 passed（+2 egress_extra 回归）+ ruff 净。
