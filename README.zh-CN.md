...
以及如何开始实现自己的 OSCA Agent。白皮书 v1.1 是文档版本；软件独立编号、现为 **v1.1**（机制可验发布：
规范 v0.4 ＋参考运行框架＋可回放脱敏样例，另含真写路径端到端接通：审批闭环 ＋ 真实 sql_readonly/openapi
执行器，测 fake 后端）已打 `v1.1` tag，真实业务与生产写验证属 1.x 旅程。
...
- **软件 v1.1——机制可验发布（已达）**：规范（v0.4）、参考实现（Runtime＋CLI）、一个可 `osca replay`
  的可回放脱敏样例账本（亲眼看判断把输出从改前移向改后），以及真写路径端到端接通（审批闭环 ＋ 真实
  sql_readonly/openapi 执行器 ＋ 可恢复剧集，测 fake 后端）。这是 `v1.1` tag 标记的东西——**不**是真实
  业务效果或生产写验证的声明（后者属 1.x／部署侧）。
- **1.x——产品成熟度验证（进行中）**：上述真实内容证据（同一真实场景 ≥20 条经专家 Confirm 入账、
  部分在独立批次再次适用）、产品界面、Creator 与生产集成；收敛曲线属 1.x 叙事。不要求公开客户原始
  账本。上面的“白皮书 v1.1”只是文档版本。
