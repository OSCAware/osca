...
以及如何开始实现自己的 OSCA Agent。白皮书 v1.1 是文档版本；软件 **v1.0**（机制可验发布：规范 v0.4
＋参考运行框架＋可回放脱敏样例）已打 `v1.0` tag，真实业务验证属 1.x 旅程。
...
- **软件 v1.0——机制可验发布（已达）**：规范（v0.4）、参考实现（Runtime＋CLI）、一个可 `osca replay`
  的可回放脱敏样例账本（亲眼看判断把输出从改前移向改后）。这是 `v1.0` tag 标记的东西——**不**是真实
  业务效果的声明。
